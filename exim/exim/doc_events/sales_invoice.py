...
def calculate_total(self):
    ...
    if (
        ...
        and self.shipping_terms not in ["CIF", "CFR", "CNF", "CPT"]
    ):
        frappe.msgprint(
            f"To calculate item wise freight please ensure shipping terms are set either of {frappe.bold('CIF, CFR, CNF OR CPT')}."
        )
...
def duty_calculation(self):
    ...
    if (
        parent_meta.has_field("total_duty_drawback")
        and frappe.db.get_value("Address", self.customer_address, "country") != "India"
    ):
        total_duty_drawback = 0.0
        for row in self.items:
            child_meta = frappe.get_meta(row.doctype)
            if (
                child_meta.has_field("duty_drawback_rate")
                and row.duty_drawback_rate
                and row.fob_value
            ):
                ...
                if child_meta.has_field("duty_drawback_amount"):
                    if row.maximum_cap == 1:
                        if row.capped_amount < duty_drawback_amount:
                            ...
                            row.effective_rate = flt(
                                row.capped_amount / row.fob_value * 100.0
                            )
                        else:
...
def validate_document_checks(self):
    if self.get("sales_invoice_export_document_item") and not all(
        [row.checked for row in self.get("sales_invoice_export_document_item")]
    ):
        frappe.throw(_("Not all documents are checked for Export Documents"))

    elif self.get("sales_invoice_contract_term_check") and not all(
        [row.checked for row in self.get("sales_invoice_contract_term_check")]
    ):
...
def export_lic(self):
    for row in self.items:
        if row.get("advance_authorisation_license"):
            aal = frappe.get_doc(
                "Advance Authorisation License", row.advance_authorisation_license
            )
            aal.append(
                "exports",
                {
...
def cancel_export_lic(self):
    doc_list = list(
        set(
            [
                row.advance_authorisation_license
                for row in self.items
